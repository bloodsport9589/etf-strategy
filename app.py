...
@st.cache_data
def run_enhanced_backtest(df_all, assets, rs, rl, rw, h, m):
...
if not df.empty:
    bt = run_enhanced_backtest(df, st.session_state.my_assets, rs, rl, rw, h, m)
    ...
    if res_df is not None:
        ...
        # --- 增强型 K 线图 ---
        st.divider()
        st.subheader("📈 策略净值走势与调仓诊断")
        
        fig = go.Figure()

        # A. 趋势背景色 (基于10日移动平均判别强弱)
