...
# ================= 1. 基础配置 =================
st.set_page_config(page_title="动能工厂-因子实验室", page_icon="🧪", layout="wide")

# 初始化参数
DEFAULTS = {
    "rs": 20, "rl": 60, "rw": 100, "h": 1, "m": 20, 
    "rsi_period": 14, 
    "rsi_limit": 80,  # 新增：RSI 阈值
    "acc_limit": -0.05 # 新增：加速度阈值
}
...
DEFAULT_ASSETS = {
    ...
    "501018.SS": "南方原油", "159985.SZ": "豆粕ETF"
}
...
def get_clean_data(assets_dict, start_date, end_date):
    ...
    try:
        ...
        for ticker in targets.keys():
            try:
                col_data = data[ticker] if isinstance(data.columns, pd.MultiIndex) else data[[ticker]]
                s = col_data['Adj Close'] if 'Adj Close' in col_data.columns else col_data['Close']
...
# 注意：这里增加了 use_rsi_filter 和 use_acc_filter 开关
def run_strategy_engine(df_all, assets, params, user_start_date, 
    ...
    for i in range(warm_up, len(df_t) - 1):
        ...
        # 最终信号 = 基础信号 & 数据有效 & 通过RSI检查 & 通过加速度检查
...
with st.sidebar:
    ...
    with st.expander("📅 时间区间", expanded=True):
        start_d = st.date_input("开始", datetime.date.today() - datetime.timedelta(days=365*3))
        end_d = st.date_input("结束", datetime.date.today())
...
if not df.empty:
    ...
    if res_base and res_new:
        ...
        c4.metric("因子触发统计", " | ".join(filter_msg) if filter_msg else "未启用过滤")
        ...
        with tab1:
            ...
            if ret_n > ret_b and abs(mdd_n) < abs(mdd_b):
                st.success(f"🎉 **因子有效！** 引入指标后，收益提升且回撤减小。RSI阈值 {rsi_limit} 和 加速度阈值 {acc_limit} 似乎是有效的参数。")
            elif ret_n < ret_b:
                st.warning(f"⚠️ **负优化**。由于过滤条件过严，错过了部分上涨行情。建议放宽阈值（如提高 RSI 上限或降低加速度下限）。")
            else:
                st.info("ℹ️ 策略表现基本持平，新因子在当前区间内触发次数较少。")

        with tab2:
            st.markdown("##### 🔍 因子作用详情")
            st.write("以下展示最近一天的所有标的数据，帮助检查为何会被过滤：")
            ...
            for name in r_score.index:
                ...
                # 重新复现逻辑
                base_cond = (r_score[name] > 0) 
                rsi_cond = (r_rsi[name] < rsi_limit)
                acc_cond = (r_acc[name] > acc_limit)
                ...
                if not base_cond:
                    status = "⚪ 无动能"
                    reason = "动能分<0 或 均线下方"
                elif use_rsi and not rsi_cond:
                    status = "⛔ RSI熔断"
                    reason = f"RSI({r_rsi[name]:.1f}) > {rsi_limit}"
                elif use_acc and not acc_cond:
                    status = "⛔ 衰竭熔断"
